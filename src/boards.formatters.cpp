...
namespace Boards
{
	...
	{
	public:
		SingleMessage_BoardFormatter(std::ostringstream& out, CHAR_DATA* character, const time_t& date, BoardTypes type, time_t& last_date) :
	...
	Boards::Board::Formatter::shared_ptr FormattersBuilder::coder(std::ostringstream& out, CHAR_DATA* character, const time_t& date)
	{
		return std::make_shared<Coder_BoardFormatter>(out, character, date);
	}

	Boards::Board::Formatter::shared_ptr FormattersBuilder::news_and_godnews(std::ostringstream& out, CHAR_DATA* character, const time_t& date)
	{
		return std::make_shared<News_And_GodNews_BoardFormatter>(out, character, date);
	}

	Boards::Board::Formatter::shared_ptr FormattersBuilder::clan_news(std::ostringstream& out, CHAR_DATA* character, const time_t& date)
	{
		return std::make_shared<ClanNews_BoardFormatter>(out, character, date);
	}

	Boards::Board::Formatter::shared_ptr FormattersBuilder::single_message(std::ostringstream& out, CHAR_DATA* character, const time_t& date, const BoardTypes type, time_t& last_date)
	{
		return std::make_shared<SingleMessage_BoardFormatter>(out, character, date, type, last_date);
	}

	Boards::Board::Formatter::shared_ptr FormattersBuilder::messages_list(std::ostringstream& out, CHAR_DATA* character, const time_t& date)
	{
		return std::make_shared<ListMessages_BoardFormatter>(out, character, date);
	}

	Boards::Board::Formatter::shared_ptr FormattersBuilder::create(const BoardTypes type, std::ostringstream& out, CHAR_DATA* character, const time_t& date)
	{
		switch (type)
		{
		case CODER_BOARD:
			return coder(out, character, date);

		case NEWS_BOARD:
		case GODNEWS_BOARD:
			return news_and_godnews(out, character, date);

		case CLANNEWS_BOARD:
			return clan_news(out, character, date);
